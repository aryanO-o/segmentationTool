...
class BitmapUtils {
    ...
    fun getBitmap(image: ImageProxy): Bitmap? {
        val frameMetadata: FrameMetadata = FrameMetadata.Builder()
            ...
            .build()
        val nv21Buffer = image.image?.let {
        ...
        return nv21Buffer?.let { getBitmap(it, frameMetadata) }
    }
...
interface ImageProxy : AutoCloseable {
    ...
    val imageInfo: ImageDecoder.ImageInfo
     ...
     * [android.media.ImageReader].  So an [Image] obtained with this method will behave
     * as such.
     *
